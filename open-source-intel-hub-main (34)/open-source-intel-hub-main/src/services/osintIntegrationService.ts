...
import { searchBreachData } from '@/services/breachService';
...
export async function processOSINTQuery(options: {
  input: string;
  includeAI?: boolean;
  deepScan?: boolean;
}): Promise<OSINTResult> {
  const { input, includeAI = true, deepScan = false } = options;
  const modulesUsed: string[] = [];
  const results: any = {};
  let riskLevel: OSINTResult['riskLevel'] = 'info';

  try {
    // 1. Internal Breach Database Search
    try {
      const breachHits = await searchBreachData(input);
      if (breachHits.length > 0) {
        results.internal_breaches = breachHits;
        modulesUsed.push('Internal Breach DB');
        riskLevel = 'critical'; // Found in our breach DB is definitely critical
      }
    } catch (e) {
      console.error('Breach search failed:', e);
    }

    // 2. Invoke Supabase Edge Function for external OSINT
    const { data, error } = await supabase.functions.invoke('phoenix-osint-analysis', {
      body: { input, includeAI, deepScan }
    });
    ...
    if (data.results) {
      ...
      // Logic to merge risk levels...
      if (data.riskLevel === 'critical') riskLevel = 'critical';
      else if (data.riskLevel === 'high' && riskLevel !== 'critical') riskLevel = 'high';
      // ... etc
    }
    
    if (data.modulesUsed) {
      modulesUsed.push(...data.modulesUsed);
    }

    // 3. Construct Final Summary
    // If we have internal breaches, prepend them to the summary
    let summary = data.summary || 'Analysis complete.';
    if (results.internal_breaches) {
      const count = results.internal_breaches.length;
      const breachText = `\n\nCRITICAL: Found ${count} matches in internal breach datasets: ${results.internal_breaches.map((b: any) => b.dataset).join(', ')}.`;
      summary = breachText + summary;
    }
    ...
    console.error('OSINT processing error:', error);
    return {
      source: 'Error',
      data: null,
      riskLevel: 'info',
      summary: 'Failed to process OSINT query.',
      recommendations: [],
      modulesUsed
    };
  }
