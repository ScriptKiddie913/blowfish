...
class FeodoTrackerService {
  ...
  async fetchActiveC2Servers(): Promise<FeodoResponse> {
    try {
      ...
      // Try the recommended blocklist first (JSON format)
      let response = await fetch(`${this.baseUrl}ipblocklist_recommended.json`, {
        headers: {
      ...
      // Fallback to regular JSON blocklist if recommended fails
      if (!response.ok) {
        console.log('[Feodo] Trying fallback endpoint...');
        response = await fetch(`${this.baseUrl}ipblocklist.json`, {
          headers: {
            'User-Agent': 'OSINT-Hub/1.0',
            'Accept': 'application/json'
          }
      ...
      // Parse Feodo JSON format - handle both array and object with data property
      const rawData = Array.isArray(data) ? data : (data.data || data);
      
      const c2Servers: C2Server[] = (Array.isArray(rawData) ? rawData : []).map((item: any, index: number) => ({
        id: `feodo-${item.ip_address || item.ip}-${item.port || 443}`,
        ip: item.ip_address || item.ip,
        port: (item.port || 443)?.toString(),
        status: item.status || 'active',
        hostname: item.hostname || item.host,
        country: item.country,
        first_seen: item.first_seen || item.firstseen || new Date().toISOString(),
        last_seen: item.last_seen || item.lastseen || new Date().toISOString(),
        malware: item.malware || item.malware_alias || item.malware_printable || 'Unknown',
        confidence: (item.confidence_level || item.confidence || 70)?.toString()
      })).filter((server: C2Server) => server.ip);
