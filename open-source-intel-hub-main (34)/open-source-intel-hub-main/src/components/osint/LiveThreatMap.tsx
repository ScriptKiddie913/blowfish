...
// Batch geolocate IPs to speed up processing
async function batchGeolocateIPs(ips: string[]): Promise<Map<string, { lat: number; lon: number; city: string; country: string }>> {
  const results = new Map();
  const uniqueIPs = [...new Set(ips)];
  const uncachedIPs = uniqueIPs.filter(ip => !geoCache.has(ip));
  ...
  // Process in batches of 45 (API rate limit is 45/min)
  // To fetch 800+ items quickly, we need to parallelize or use a better API
  // For now, we'll try to burst as much as possible with Promise.all
  
  // Since we need 800+ results, we can't rely solely on the free API for everything sequentially.
  // We will try to fetch 10 at a time with a small delay.
...
async function fetchFeodoAttacks(): Promise<LiveAttack[]> {
  ...
  try {
    const res = await fetch('/api/feodo/ipblocklist_recommended.json');
    if (!res.ok) return [];
    const data = await res.json();
    const items = Array.isArray(data) ? data : [];
    
    // Process top 200 items for speed/volume balance
    const topItems = items.slice(0, 200);
    const ipsToLocate = topItems.map((i: any) => i.ip_address || i.ip).filter(Boolean);
    const geoMap = await batchGeolocateIPs(ipsToLocate);
...
async function fetchURLhausAttacks(): Promise<LiveAttack[]> {
  const attacks: LiveAttack[] = [];
  try {
    const res = await fetch('/api/urlhaus-api/urls/recent');
    if (!res.ok) return [];
    const data = await res.json();
    const items = data?.urls || [];
    
    // Process top 200 items
    const topItems = items.slice(0, 200);
    const hostsToLocate: string[] = [];
    
    // Pre-process URLs to get hostnames
    const validItems = topItems.filter((item: any) => {
      try {
        const url = new URL(item.url);
        hostsToLocate.push(url.hostname);
        return true;
      } catch {
        return false;
      }
    ...
    const geoMap = await batchGeolocateIPs(hostsToLocate);

    for (const item of validItems) {
      try {
        const url = new URL(item.url);
        const host = url.hostname;
        const geo = geoMap.get(host);
...
async function fetchThreatFoxAttacks(): Promise<LiveAttack[]> {
  const attacks: LiveAttack[] = [];
  try {
    const res = await fetch('/api/threatfox-api', {
      method: 'POST',
    ...
    for (const item of validItems) {
      ...
      const ipMatch = ioc.match(/(\d{1,3}\.){3}\d{1,3}/);
      if (ipMatch) {
        const geo = geoMap.get(ipMatch[0]);
        if (geo) {
          const target = TARGET_LOCATIONS[Math.floor(Math.random() * TARGET_LOCATIONS.length)];
...
async function fetchRansomwareLiveAttacks(): Promise<LiveAttack[]> {
  ...
  try {
    ...
    const ipsToLocate = topItems.map((i: any) => i.ip || i.victim_ip).filter(Boolean);
    const geoMap = await batchGeolocateIPs(ipsToLocate);
...
// Fetch APT groups from APTmap
async function fetchAPTGroups(): Promise<LiveAttack[]> {
  const attacks: LiveAttack[] = [];
  try {
    ...
    aptData.forEach(apt => {
      ...
      attacks.push({
        ...
        malwareFamily: apt.ttps.join(', '),
        progress: 0,
      });
    });
  } catch (err) {
    console.error('[APTmap] Error:', err);
  }
  return attacks;
}
...
export function LiveThreatMap() {
  ...
  const [viewMode, setViewMode] = useState<'2d' | '3d' | 'flight'>('3d'); // Default to 3D globe
          ...
          <CardHeader className="pb-2">
            <CardTitle className="flex items-center justify-between">
              <span className="flex items-center gap-2">
                ...
                {viewMode === '3d' ? '3D Interactive Globe' : viewMode === 'flight' ? 'Real-Time Flight Tracker' : 'Global Attack Distribution'}
              </span>
              {lastUpdate && (
          ...
          <CardContent>
            {viewMode === '3d' ? (
              ...
              <div className="relative w-full" style={{ height: '600px' }}>
                <iframe
                  src="https://sotanik-flight-radar.vercel.app/"
